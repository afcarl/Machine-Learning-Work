# Liver Dataset
#============================================================================
ds = read.table("https://archive.ics.uci.edu/ml/machine-learning-databases/liver-disorders/bupa.data",sep=",")
...
mse = function(){
  sample <- read.table("https://archive.ics.uci.edu/ml/machine-learning-databases/liver-disorders/bupa.data",sep=",")
  y = (matrix(c(ifelse(sample$V7==2, 0, 1))))
  sample = cbind(sample[,-7], y )  
  op_mean_list = list()
  length_list = list()
  for(i in 20:90){    
    size1 = i/100   
    t.idx<-sample(1:345, size = size1*345)
    sample.tr<-sample[t.idx,]
    sample.te<-sample[-t.idx,]    
    logit1<-glm(y~., sample.tr, family=binomial())
    logit1
    summary(logit1)    
