# Sonar Dataset
#============================================================================
ds = read.table(file="sonar.all-data", header=FALSE, sep=",")
#================================== 
...
mse = function(){
  ...
  for(i in 1:90){   
    size1 = i/100
    t.idx<-sample(1:208, size = size1 * 208) 
    ...
    length = length(Sonar.tr$Class)
    length_list = c(length_list, length)
    pred_class = (matrix(c(ifelse(prob>0.5, "R", "M"))))    
    table(Class=Sonar.te$Class, pred=pred_class)
    op_mean <<- mean(Sonar.te$Class != pred_class) 
