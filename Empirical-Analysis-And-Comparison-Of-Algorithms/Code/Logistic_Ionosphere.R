# Ionosphere Dataset
#============================================================================
ds = read.table("https://archive.ics.uci.edu/ml/machine-learning-databases/ionosphere/ionosphere.data",sep=",")
...
mse = function(){
  ...
  for(i in 1:90){   
    size1 = i/100
    t.idx<-sample(1:351, size = size1 * 351)
    sample.tr<-sample[t.idx,]
    sample.te<-sample[-t.idx,]   
    logit1<-glm(V35~., sample.tr, family=binomial())
    logit1
    summary(logit1)    
    prob<<-predict(logit1, sample.te, type="response")
    length = length(sample.tr$V35)
    length_list = c(length_list, length)   
    pred_class = (matrix(c(ifelse(prob>0.5, "g", "b"))))    
    table(Class=sample.te$V35, pred=pred_class)
    op_mean = mean(sample.te$V35 != pred_class)
  ...
  plot(y,x, pch=".", col="blue", main = "Logistic Regression Error \n Ionosphere", xlab = "Training data length", ylab = "Mean Error")
