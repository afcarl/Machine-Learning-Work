...
# ================================================================================================================================
m_dataset1 = read.table(file="mvar-set1.dat", header=FALSE)
attach(m_dataset1)
print(m_dataset1)
plot(m_dataset1)
# ================================================================================================================================
...
split_fn = function(split_size){
  splitds = sample(1:nrow(m_dataset1), size=split_size*nrow(m_dataset1))
  train_set <<- m_dataset1[splitds,]
  print(train_set)
  test_set <<- m_dataset1[-splitds,]
  print(test_set)
}
...
grad_descent = function(train){
  ...
  for(n in 1:200){ #Performing 200 iterations
    for(i in 1:length(train)){
      ...
      y_hat = (theta_0+(theta_1*V1)+theta_2*V2)
      error = ((y_hat-V3[i])^2)
      #   error = ((1/(2*length(train)))*((y_hat-V2[i])^2))
...
gaus = function(train){
  d = density((V1+V2), bw = 0.5, kernel="gaussian")
