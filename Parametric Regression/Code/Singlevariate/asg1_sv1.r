...
# ================================================================================================================================
dataset1 = read.table(file="svar-set1.dat", header=FALSE)
attach(dataset1)
print(dataset1)
plot(dataset1)
# ================================================================================================================================
...
split_fn = function(split_size){
  splitds = sample(1:nrow(dataset1), size=split_size*nrow(dataset1))
  train_set <<- dataset1[splitds,]
  print(train_set)
  test_set <<- dataset1[-splitds,]
  print(test_set)
}
...
lin_train = function(train){
  ...
  y_hat1 <<- res0 + (res1*V1) #Linear regression hypothesis
  print (y_hat1)
  plot (y_hat1,V1)
  summary(y_hat1)
  
  mean_train_error <<- (mean((y_hat1-V2)^2)) #Mean Square Error cost function
...
lin_test = function(test){
  ...
  y_hat2 <<- res0 + (res1*V1) #Linear regression hypothesis
  print (y_hat2)
  plot (y_hat2,V1)
  summary(y_hat2)
  
  mean_test_error <<- (mean((y_hat2-V2)^2)) #Mean Square Error cost function
...
#Using quadratic model on training set
# ================================================================================================================================

poly_train = function(train){
  attach(train)
  train_poly = lm(V2~(V1+I(V1^2)))
  plot(train_poly)
  summary(train_poly)
...
#Using quadratic model on test set
# ================================================================================================================================

poly_test = function(test){
  attach(test)
  test_poly = lm(V2~(V1+I(V1^2)))
  plot(test_poly)
  summary(test_poly)
  plot(V1, V2, type="p", lwd=1)
