...
# ================================================================================================================================
dataset3 = read.table(file="svar-set3.dat", header=FALSE)
attach(dataset3)
print(dataset3)
plot(dataset3)
# ================================================================================================================================
...
split_fn = function(split_size){
  splitds = sample(1:nrow(dataset3), size=split_size*nrow(dataset3))
  train_set <<- dataset3[splitds,]
  print(train_set)
  test_set <<- dataset3[-splitds,]
  print(test_set)
}
...
#Using degree=8 model on training set
# ================================================================================================================================

poly_train = function(train){
  attach(train)
  train_poly = lm(V2~(V1+I(V1^2)+I(V1^3)+I(V1^4)+I(V1^5)+I(V1^6)+I(V1^7)+I(V1^8)))
  plot(train_poly)
  summary(train_poly)
  plot(V1, V2, type="p", lwd=1)
  points(V1, predict(train_poly), type="l", col="red", lwd=2)
  train_pol = function(x){
    poly_model8$coefficient[9]*x^8 +poly_model8$coefficient[8]*x^7 +poly_model8$coefficient[7]*x^6 +poly_model8$coefficient[6]*x^5 +poly_model8$coefficient[5]*x^4 +poly_model8$coefficient[4]*x^3 +poly_model8$coefficient[3]*x^2 + poly_model8$coefficient[2]*x + poly_model8$coefficient[1]
  }
...
#Using degree=8 on test set
# ================================================================================================================================

poly_test = function(test){
  attach(test)
  test_poly = lm(V2~(V1+I(V1^2)+I(V1^3)+I(V1^4)+I(V1^5)+I(V1^6)+I(V1^7)+I(V1^8)))
  plot(test_poly)
  summary(test_poly)
  plot(V1, V2, type="p", lwd=1)
